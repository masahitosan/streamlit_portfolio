...
fig.update_layout(title='月別売上と利益',    xaxis_title='月', yaxis_title='金額(万円)')
st.plotly_chart(fig)
...
if uploaded_csv is not None:
    ...
    selected_column = st.selectbox("グラフ化する列を選んでください", numeric_columns, key="csv_column_select")
...
# 3. 異なる幅のカラム
# カラムの幅を調整して、より柔軟なレイアウトを作成する⽅法を⽰します。
...
# 4. コンテナの使⽤
#関連する要素をグループ化してレイアウトを整理する⽅法を学びます。
with st.container():
...
# 5. ネストされたレイアウト
#カラムとコンテナを組み合わせて、より複雑なレイアウトを作成する⽅法を⽰します。
with st.container():
...
with st.expander("統計情報"):
    ...
    st.write(f"最高売上: {sales_data['売上'].max():.2f}円")
    st.write(f"最低売上: {sales_data['売上'].min():.2f}円")

st.subheader("3. サイドバーの使⽤例")
